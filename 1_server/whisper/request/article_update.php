...
if (!check_ip($IP_WHITE_LIST)){
...
else if (is_empty($id) || is_empty($type) || is_empty($content)){
    $msg = "参数错误";
}
else {
    ...
    if ($mysqli->connect_errno){
        ...
        $stmt=$mysqli->prepare("UPDATE article SET type_id = ?, content = ? WHERE id = ?");
        $stmt->bind_param('isi', $row['id'], $content, $id);
        $stmt->execute();
        $status = 0;
        $msg = "修改成功";
    }
